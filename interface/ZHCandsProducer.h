#ifndef ANALYSIS_VLQANA_ZHCANDSPRODUCER_HH
#define ANALYSIS_VLQANA_ZHCANDSPRODUCER_HH
...
class ZHCandsProducer {
  public:


  void operator()(int N, int K, vlq::JetCollection& goodJets, vlq::CandidateCollection& ZH)
  {
...
//if(A[2]<120. && A[2]>240.){cout<< "Total mass of 3 jets  is ******  = " << A[2]<<endl; B.push_back(A[2]);}
    ...
    for ( unsigned j=0; j<D.size(); j++){
      ...
      if (D.at(j).Pt()>100. && D.at(j).Mag()>=70 && D.at(j).Mag()<=160){

	//	cout << " j the element is *********" << D.at(j).Mag()<<endl;
       	TLorentzVector ZHp4 = D.at(j);
	vlq::Candidate zh(ZHp4) ;
